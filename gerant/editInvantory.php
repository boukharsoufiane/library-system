...
if (isset($_POST["edit"])) {
  ...
  if (!empty($pages_ouvrage)) {
    if(!empty($image_main)){
      $sql = "UPDATE ouvrage SET name_ouvrage='$name_ouvrage', state_ouvrage='$state_ouvrage', date_achat='$date_achat', date_edition='$date_edition', type_ouvrage='$type_ouvrage', pages_ouvrage='$pages_ouvrage', image_main='$gerant_folder' WHERE id_ouvrage='$id'";
    } else {
      $sql = "UPDATE ouvrage SET name_ouvrage='$name_ouvrage', state_ouvrage='$state_ouvrage', date_achat='$date_achat', date_edition='$date_edition', type_ouvrage='$type_ouvrage', pages_ouvrage='$pages_ouvrage' WHERE id_ouvrage='$id'";
    }
  } else {
    $sql = "UPDATE ouvrage SET name_ouvrage='$name_ouvrage', state_ouvrage='$state_ouvrage', date_achat='$date_achat', date_edition='$date_edition', type_ouvrage='$type_ouvrage' WHERE id_ouvrage='$id'";
  }

  if (mysqli_query($conn, $sql)) {
    $query = "INSERT INTO gestion (id_ouvrage, id_gerant, type_operation, date_operation) VALUES('$id','$id_gerant', 'Edit', NOW())";
  ...
  <div class="container">
    <div class="overlay" id="overlay">
      <div class="sign-in" id="sign-in">
        <h1>Welcome Back!</h1>
        <p>To edit book or other thing please enter all infomation</p>
    ...
    <div class="form">
      <div class="sign-up" id="sign-up-info">
        <h1 style="margin-bottom: 2%;">Edit Information</h1>
        <?php
        session_start();
        $id = $_GET['id'];
        ...
        ?>
          <form id="sign-up-form" method="post" enctype="multipart/form-data">
            <input type="text" placeholder="Name" value="<?php echo $row['name_ouvrage']; ?>" name="name_ouvrage" />
            <input type="date" placeholder="Date of buy" value="<?php echo $row['date_achat']; ?>" name="date_achat" />
            <input type="date" placeholder="Date of edition" value="<?php echo $row['date_edition']; ?>" name="date_edition" />
            <div style="display: flex;margin-left:22%">
              <select name="state_ouvrage">
                <option value="<?php echo $row['state_ouvrage']; ?>"><?php echo $row['state_ouvrage']; ?></option>
                <option value="EXCELLENT">EXCELLENT</option>
                <option value="MEDUIM">MEDUIM</option>
              </select>
              <select name="type_ouvrage" style="margin-left: 3%;" onchange="showType(this)">
                <option value="<?php echo $row['type_ouvrage']; ?>"><?php echo $row['type_ouvrage']; ?></option>
            ...
            <input type="number" id="pages" style="display: none;margin-left:15%" value="<?php echo $row['pages_ouvrage']; ?>" placeholder="Pages" name="pages_ouvrage" />
            <input type="file" name="image_main" />
            <button class="control-button up" type="submit" name="edit">Edit</button>
          </form>
        <?php
        }
        ?>
